<template>
    <div>
        ...
        >
                ...
                <Tabs value="detail">
                    <TabPane :label="L('RoleDetails')" name="detail">
                        <FormItem :label="L('RoleName')" prop="name">
                            <Input name="add_roleName_input" v-model="role.name" :maxlength="32" :minlength="2"></Input>
                        </FormItem>
                        <FormItem :label="L('DisplayName')" prop="displayName">
                            <Input name="add_roleDisplayName_input" v-model="role.displayName" :maxlength="32" :minlength="2"></Input>
                        </FormItem>
                        <FormItem :label="L('Description')" prop="description">
                            <Input name="add_roleDescription_input" v-model="role.description" :maxlength="1024"></Input>
                        </FormItem>
                      </TabPane>
            ...
            <div slot="footer">
                <Button name="add_role_cancel" @click="cancel">{{L('Cancel')}}</Button>
                <Button name="add_role_save" @click="save" type="primary">{{L('OK')}}</Button>
            </div>
        </Modal>
    </div>
</template>
