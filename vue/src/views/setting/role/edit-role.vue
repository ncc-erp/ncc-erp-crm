                ...
                <Tabs value="detail">
                    <TabPane :label="L('RoleDetails')" name="detail">
                        <FormItem :label="L('RoleName')" prop="name">
                            <Input name="edit_roleName_input" v-model="role.name" :maxlength="32" :minlength="2"></Input>
                        </FormItem>
                        <FormItem :label="L('DisplayName')" prop="displayName">
                            <Input name="edit_roleDisplayName_input" v-model="role.displayName" :maxlength="32" :minlength="2"></Input>
                        </FormItem>
                        <FormItem :label="L('Description')" prop="description">
                            <Input name="edit_roleDescription_input" v-model="role.description" :maxlength="1024"></Input>
                        </FormItem>
                      </TabPane>
                    <TabPane :label="L('RolePermission')" name="permission">
                        <CheckboxGroup v-model="role.grantedPermissions">
                            <Checkbox :name="`role${permission.name}_input`" :label="permission.name" v-for="permission in permissions" :key="permission.name"><span>{{permission.displayName}}</span></Checkbox>
                        </CheckboxGroup>
                    </TabPane>
                </Tabs>
            </Form>
            <div slot="footer">
                <Button name="edit_role_cancel" @click="cancel">{{L('Cancel')}}</Button>
                <Button name="edit_role_save" @click="save" type="primary">{{L('OK')}}</Button>
            </div>
        </Modal>
    </div>
</template>
