        ...
        >
            <Form ref="tenantForm"  label-position="top" :rules="tenantRule" :model="tenant">
                <FormItem :label="L('TenancyName')" prop="tenancyName">
                    <Input name="add_tenantTenancyName_input" v-model="tenant.tenancyName" :maxlength="32" :minlength="2"></Input>
                </FormItem>
                <FormItem :label="L('Name')" prop="name">
                    <Input name="add_tenantName_input" v-model="tenant.name" :maxlength="32" :minlength="2"></Input>
                </FormItem>
                <FormItem :label="L('DatabaseConnectionString')" prop="databaseConnectionString">
                    <Input name="add_tenantDatabaseConnectionString_input" v-model="tenant.databaseConnectionString" :maxlength="1024"></Input>
                </FormItem>
                <FormItem :label="L('AdminEmailAddress')" prop="adminEmailAddress">
                    <Input name="add_tenantAdminEmailAddress_input" v-model="tenant.adminEmailAddress" type="email" :maxlength="256"></Input>
                </FormItem>
                <FormItem>
                    <Checkbox name="add_tenantIsActive_input" v-model="tenant.isActive">{{L('IsActive')}}</Checkbox>
                </FormItem>
                <p><p>{{L("DefaultPasswordIs",undefined,'123qwe')}}</p></p>
            </Form>
            <div slot="footer">
                <Button name="add_tenant_cancel" @click="cancel">{{L('Cancel')}}</Button>
                <Button name="add_tenant_save" @click="save" type="primary">{{L('OK')}}</Button>
            </div>
        </Modal>
    </div>
</template>
