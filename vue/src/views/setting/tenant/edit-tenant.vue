        ...
        >
            <Form ref="tenantForm"  label-position="top" :rules="tenantRule" :model="tenant">
                <FormItem :label="L('TenancyName')" prop="tenancyName">
                    <Input name="edit_tenantTenancyName_input" v-model="tenant.tenancyName" :maxlength="32" :minlength="2"></Input>
                </FormItem>
                <FormItem :label="L('Name')" prop="name">
                    <Input name="edit_tenantName_input" v-model="tenant.name" :maxlength="32" :minlength="2"></Input>
                </FormItem>
                <FormItem>
                    <Checkbox name="edit_tenantIsActive_input" v-model="tenant.isActive">{{L('IsActive')}}</Checkbox>
                </FormItem>
            </Form>
            <div slot="footer">
                <Button name="edit_tenant_cancel" @click="cancel">{{L('Cancel')}}</Button>
                <Button name="edit_tenant_save" @click="save" type="primary">{{L('OK')}}</Button>
            </div>
        </Modal>
    </div>
</template>
