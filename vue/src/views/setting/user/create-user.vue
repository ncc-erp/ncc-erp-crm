<template>
    <div>
        ...
        >
                ...
                <Tabs value="detail">
                    <TabPane :label="L('UserDetails')" name="detail">
                        <FormItem :label="L('UserName')" prop="userName">
                            <Input name="add_userUserName_input" v-model="user.userName" :maxlength="32" :minlength="2"></Input>
                        </FormItem>
                        <FormItem :label="L('Name')" prop="name">
                            <Input name="add_userName_input" v-model="user.name" :maxlength="32"></Input>
                        </FormItem>
                        <FormItem :label="L('Surname')" prop="surname">
                            <Input name="add_userSurname_input" v-model="user.surname" :maxlength="1024"></Input>
                        </FormItem>
                        <FormItem :label="L('EmailAddress')" prop="emailAddress">
                            <Input name="add_userEmailAddress_input" v-model="user.emailAddress" type="email" :maxlength="32"></Input>
                        </FormItem>
                        <FormItem :label="L('Password')" prop="password">
                            <Input name="add_userPassword_input" v-model="user.password" type="password" :maxlength="32"></Input>
                        </FormItem>
                        <FormItem :label="L('ConfirmPassword')" prop="confirmPassword">
                            <Input name="add_userConfirmPassword_input" v-model="user.confirmPassword" type="password" :maxlength="32"></Input>
                        </FormItem>
                        <FormItem>
                            <Checkbox name="add_userIsActive_input" v-model="user.isActive">{{L('IsActive')}}</Checkbox>
                        </FormItem>
                    </TabPane>
            ...
            <div slot="footer">
                <Button name="add_user_cancel" @click="cancel">{{L('Cancel')}}</Button>
                <Button name="add_user_save" @click="save" type="primary">{{L('OK')}}</Button>
            </div>
        </Modal>
    </div>
</template>
